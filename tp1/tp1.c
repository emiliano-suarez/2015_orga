...
int sha1(char *resultado, char *nombre_archivo, unsigned long longitudOriginal);
unsigned int leftrotate(unsigned int valor,int desplazamiento);
long int calcularTamanioArchivo(char* nombreFile);
void asignarDatos(char *primerCaracter, unsigned char *bloques,long int tamanioOriginal, long int longitudRelleno);
...
int sha1(char *resultado, char *primerCaracter, unsigned long longitudOriginal)
{
    ...
    asignarDatos(primerCaracter, bloques, longitudOriginal, longitudRelleno); // se almacena el tamanioOrginal al final
    ...
    while(cantBloques--) {
        ...
        for(i=0; i < 80; i++) {
            if( i <= 19) { //0 ≤ i ≤ 19
                f = (b & c) ^ ((~b) & d);
                k = 0x5A827999;
            } else if ( (i>=20) && (i <= 39)) { //20 ≤ i ≤ 39
                f = b ^ c ^ d;
                k = 0x6ED9EBA1;
            } else if ( (i >= 40) && (i <= 59)) { //40 ≤ i ≤ 59
                f = (b & c) | (b & d) | (c & d);
                k = 0x8F1BBCDC;
            }
            else if ( (i>=60)&&(i<=79)) { //60 ≤ i ≤ 79
                f = b ^ c ^ d;
                k = 0xCA62C1D6;
            }
...
void asignarDatos(char *primerCaracter, unsigned char *bloques,long int tamanioOriginal, long int longitudRelleno)
{
    ...
    for (i = 0; i < tamanioOriginal; i++) {
        *(bloques + indice) = (int)primerCaracter;
        indice++;
        primerCaracter++;
    }
